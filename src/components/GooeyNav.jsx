...
const GooeyNav = ({
  ...
  const [activeIndex, setActiveIndex] = useState(
    typeof controlledActiveIndex === 'number' ? controlledActiveIndex : initialActiveIndex
  );
  ...
  const triggerTransition = (targetLi, index) => {
    ...
    if (filterRef.current) {
      ...
      particles.forEach(p => {
        if (p.parentNode === filterRef.current) {
  ...
  // Sync with controlledActiveIndex from scroll tracking
  useEffect(() => {
    if (typeof controlledActiveIndex === 'number' && controlledActiveIndex !== activeIndex) {
      const activeLi = navRef.current?.querySelectorAll('li')[controlledActiveIndex];
      if (activeLi) {
        triggerTransition(activeLi, controlledActiveIndex);
      } else {
