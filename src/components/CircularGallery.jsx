...
class App {
  ...
  setupIntersectionObserver() {
    this.observer = new IntersectionObserver(
      ([entry]) => {
        ...
        if (this.isIntersecting && !this.raf) {
          this.update();
        }
      },
      { threshold: 0.05 }
  ...
  onTouchUp(e) {
    if (this.isDown) {
      ...
      const dist = Math.abs((this.start || 0) - (endX || 0));
      const duration = Date.now() - (this.startTime || 0);
      
      // If it was a quick click/tap without dragging
      if (dist < 10 && duration < 400 && this.medias && this.medias.length > 0) {
        const rect = this.container.getBoundingClientRect();
        const ndcX = ((endX - rect.left) / rect.width) * 2 - 1;
        const clickWorldX = ndcX * (this.viewport.width / 2);
        
        let clickedMedia = null;
        let minClickDist = Infinity;
        this.medias.forEach(media => {
          const halfW = (media.plane.scale.x / 2) * 1.3;
          const d = Math.abs(media.plane.position.x - clickWorldX);
          if (d <= halfW && d < minClickDist) {
            minClickDist = d;
            clickedMedia = media;
          }
        });

        if (clickedMedia && clickedMedia.itemData) {
          this.scroll.target += clickedMedia.plane.position.x;
          if (this.onItemClick) {
            this.onItemClick(clickedMedia.itemData);
          }
        } else {
          let closestMedia = null;
          let minOffset = Infinity;
          this.medias.forEach(media => {
            const offset = Math.abs(media.plane.position.x);
            if (offset < minOffset) {
              minOffset = offset;
              closestMedia = media;
            }
          });
          if (closestMedia && closestMedia.itemData) {
            if (this.onItemClick) {
              this.onItemClick(closestMedia.itemData);
            }
          }
        }
      }
  ...
  goToId(id) {
    ...
    this.medias.forEach(m => {
      if (m.itemData?.id === id) {
        const diff = Math.abs(m.plane.position.x);
        if (diff < minDiff) {
          minDiff = diff;
          target = m;
        }
    ...
    if (target) {
      this.scroll.target += target.plane.position.x;
      this.onCheckDebounce();
    }
  }
  onWheel(e) {
    // Only capture wheel inside container
  ...
  update() {
    ...
    if (this.medias) {
      ...
      if (closestMedia && closestMedia.itemData && closestMedia.itemData.id !== this.lastActiveId) {
        this.lastActiveId = closestMedia.itemData.id;
        if (this.onActiveChange) {
          this.onActiveChange(closestMedia.itemData);
        }
      }
    }
  ...
  addEventListeners() {
    ...
    // Listen for drag interactions on container and mouse movement
...
const CircularGallery = forwardRef(function CircularGallery({
  ...
  useImperativeHandle(ref, () => ({
    ...
    goToId: (id) => appRef.current?.goToId(id)
  }));
  ...
  }, [items, bend, textColor, borderRadius, font, fontUrl, scrollSpeed, scrollEase, onItemClick, onActiveChange]);
